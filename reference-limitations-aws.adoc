...
+
BlueXP에서 볼륨을 생성할 때 스토리지 효율성을 선택하지 않거나 볼륨을 생성한 후 생성할 수 있습니다 http://["CLI를 사용하여 데이터 압축을 비활성화합니다"^].
...
Cloud Volumes ONTAP는 유휴 상태일 때는 CPU를 방출하지 않으므로 https://["Amazon CloudWatch 를 참조하십시오"^] 는 사용량이 100%로 표시되므로 EC2 인스턴스에 대해 높은 CPU 경고를 보고할 수 있습니다. 이 알람을 무시할 수 있습니다. ONTAP statistics 명령은 CPU의 실제 사용량을 표시합니다.
...
https://["AWS에서 실행 중인 Cloud Volumes ONTAP HA 쌍에서 스토리지의 작동 방식에 대해 알아보십시오"^].
